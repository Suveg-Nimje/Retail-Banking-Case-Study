...
#Inserting values at branch master.
insert into branch_mstr values (1, "Delhi"), (2, "Mumbai");

insert into customer values (1,	'Ramesh', 'Chandra', 'Sharma', 'Service', '1976-12-06'),
(2,	'Avinash', 'Sunder', 'Minha', 'Business','1974-10-16');

insert into accounts values (1, 1, 1, 10000,  'Saving', '2012-12-15', 'Active'),
...
select * from accounts where open_dt between '2012-07-01' AND '2013-01-01';
...
select * from accounts order by curbal limit 1;

-- 11.	Give the count of customer for each occupation
select occupation, count(*) from customer group by occupation;

-- 12.	Write a query to find the name (first_name, last_name) of the employees who are managers.
select * from employee where desig like'%manager%';
...
select custid, fname, bid, curbal from customer join accounts using (custid);

-- 17.	Create a virtual table to store the customers who are having the accounts in the same city as they live
select * from customer join accounts using (custid) join branch_mstr on bid = branch_no
where customer.City = branch_mstr.city;
...
create table transactions(
    ...
    primary key(tid),
    foreign key(custid) references customer(custid),
    foreign key(account_no) references accounts(acnumber),
    foreign key(bid) references branch_mstr(branch_no),
    CONSTRAINT atype CHECK (atype = 'Deposite' or atype='Withdraw')
);
...
insert into transactions value(null, 2, 2, 2, 2000, 'Deposite', curdate());

-- 19.	Write a query to display the details of customer with second highest balance 
select * from customer join accounts using (custid) order by curbal desc limit 1,2;

-- 20.	Take backup of the databse created in this case study.
-- Took the backup of data base.
